...
$stmt = $db->prepare("
    SELECT c.*, ay.year as academic_year, u.first_name, u.last_name
    FROM courses c
...
// Build the leaderboard query based on selected section
$where_conditions = ["cs.course_id = ?"];
$params = [$course_id];
...
$leaderboard_sql = "
    SELECT 
        ...
        (SELECT COUNT(*) FROM module_progress mp 
         ...
         WHERE mp.student_id = u.id AND mp.is_completed = 1 AND cm.course_id = ?) as completed_modules,
        (SELECT COUNT(*) FROM video_views vv 
         JOIN course_videos cv ON vv.video_id = cv.id 
         JOIN course_modules cm ON cv.module_id = cm.id 
         WHERE vv.student_id = u.id AND cm.course_id = ?) as watched_videos,
        (SELECT AVG(aa.score) FROM assessment_attempts aa 
         JOIN assessments a ON aa.assessment_id = a.id 
         JOIN course_modules cm ON a.module_id = cm.id 
         WHERE aa.student_id = u.id AND cm.course_id = ? AND aa.status = 'completed') as average_score,
        (SELECT COUNT(*) FROM assessment_attempts aa 
         JOIN assessments a ON aa.assessment_id = a.id 
         JOIN course_modules cm ON a.module_id = cm.id 
         WHERE aa.student_id = u.id AND cm.course_id = ? AND aa.score >= 70 AND aa.status = 'completed') as high_scores,
        (
            (SELECT COUNT(*) FROM module_progress mp 
             JOIN course_modules cm ON mp.module_id = cm.id 
             WHERE mp.student_id = u.id AND mp.is_completed = 1 AND cm.course_id = ?) * 10 +
...
$stmt->execute(array_merge($params, [$course_id, $course_id, $course_id, $course_id, $course_id, $course_id]));
...
if ($total_students > 0) {
    $stmt = $db->prepare("
        SELECT 
            (SELECT COUNT(*) FROM course_modules WHERE course_id = ?) as total_modules,
            (SELECT COUNT(*) FROM course_videos WHERE module_id IN (SELECT id FROM course_modules WHERE course_id = ?)) as total_videos,
            (SELECT COUNT(*) FROM assessments WHERE module_id IN (SELECT id FROM course_modules WHERE course_id = ?)) as total_assessments
    ");
